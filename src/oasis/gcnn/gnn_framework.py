...
class GraphSample:
    ...
    """
    def __init__(self, x: torch.Tensor, edge_index: torch.Tensor, edge_type: torch.Tensor, node_type: torch.Tensor, y: Optional[torch.Tensor] = None, atoms=None):
        """
        Args:
...
class FeatureBuilder:
    ...
    Currently: atomic number Z embedding only.
    To extend: add per-type feature functions.
    """
    def __init__(self, embedding_dim: int = 16):
        self.embedding_dim = embedding_dim
        self.z_embed = nn.Embedding(100, embedding_dim)  # Z up to 99
    ...
    def build_features(self, atoms, node_type: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            raw_features: Tensor [num_nodes, feature_dim]
        """
        z = torch.tensor([atom.number for atom in atoms], dtype=torch.long)
        return self.z_embed(z)  # [num_nodes, embedding_dim]
