...
class FeatureBuilder(nn.Module):
    ...
    Now: [group embedding | period embedding | metal physics (4)]
    """
    def __init__(self, group_dim=8, period_dim=8):
        super().__init__()
        ...
        self.extra_dim = 4
        self.output_dim = group_dim + period_dim + self.extra_dim

    def build_features(self, sample: GraphSample) -> torch.Tensor:
         atoms = sample.atoms
         device = next(self.group_embed.parameters()).device

         group = torch.tensor(
                    [atom.group for atom in atoms], dtype=torch.long, device=device
              )
         period = torch.tensor(
                   [atom.period for atom in atoms], dtype=torch.long, device=device
            )
         ...
         return torch.cat([group_feat, period_feat, physics], dim=1)
