...
def learning_curve_plot(
    ...
    moe_train_fracs: Sequence[float] | None = None,
) -> Path:
    """
    Reproduce the ensemble RMSE sweeps from the notebook's final cell and plot the overlay.

    If ``moe_train_fracs`` are provided, the MoE gating model is trained/evaluated on
    those train fractions and the resulting test RMSE values are plotted at the
    corresponding train-set sizes on the same x-axis.
    """
    ...
    use_moe = cfg.plot.use_moe if cfg else True
    ...
    moe_results = None
    if use_moe and moe_train_fracs:
        if cfg is None:
            raise ValueError("Config is required to run MoE evaluations.")
        from oasis.processing import moe_split_metrics

        moe_results = moe_split_metrics(cfg, moe_train_fracs)
    ...
    if moe_results is not None:
        moe_pd = (
            moe_results.to_pandas()
            if isinstance(moe_results, pl.DataFrame)
            else pd.DataFrame(moe_results)
        )
        if not moe_pd.empty:
            missing_cols = {"n_train", "test_rmse"} - set(moe_pd.columns)
            if missing_cols:
                raise ValueError(
                    f"MoE results are missing required columns: {', '.join(sorted(missing_cols))}"
                )
            ax.scatter(
                moe_pd["n_train"],
                moe_pd["test_rmse"],
