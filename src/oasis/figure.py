...
def horizontal_panel_figure(
    ...
    *,
    ...
    images = [plt.imread(path) for path in resolved_paths]
    width_ratios = [image.shape[1] / max(image.shape[0], 1) for image in images]
    fig_width = max(6.0 * len(images), 4.0 * sum(width_ratios))
    fig_height = max(4.0, fig_width / max(sum(width_ratios), 1.0))
    fig, axes = plt.subplots(
        1,
        len(images),
