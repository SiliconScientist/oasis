...
class LearningCurveTimeAccuracyTests(unittest.TestCase):
    ...
    def test_build_mean_time_accuracy_table_averages_one_row_per_method(self) -> None:
        results = LearningCurveResults(
            ridge_df=pd.DataFrame(
                {
                    "n_train": [4, 8],
        ...
        self.assertEqual(table["method"].tolist(), ["ridge", "weighted_linear"])
        self.assertEqual(table["rmse_mean"].tolist(), [0.365, 0.3])
        self.assertEqual(table["generation_time_s"].tolist(), [15.0, 15.0])
        self.assertEqual(table["training_time_s"].tolist(), [0.23, 0.135])
        self.assertEqual(table["total_time_s"].tolist(), [15.23, 15.135])
